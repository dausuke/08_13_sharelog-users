$(function () {
    ...
    $('.post_comment').on('click', function () {
        //コメント欄の情報を変数commentに格納
        const comment = $(this).parents('.timelineitem').find('.comment');
        //コメントの投稿先を変数commenttargetに格納
        const commnenttarget = $(this)
            .parents('.timelineitem')
            .find('.commentarea');
        //投稿されているデータのid
        const contentid = $(this)
            ...
            .find('.content_id')
            .text();
        // const commenttarget = $(this).parents('.timelineitem').find
        if (comment.hasClass('appear')) {
            comment.removeClass('appear');
            $('.commentcontent').remove();
        } else {
            comment.addClass('appear');
            $.ajax({
                //POST通信
                type: 'POST',
                url: 'comment_read.php',
            }).then(
                // 通信成功時
                function (data) {
                    const respons_commentdata = JSON.parse(data);
                    // console.log(respons_commentdata);
                    respons_commentdata.forEach(function (value, key) {
                        const commentTag = `
                                <div class="commentcontent">
                                    <p>${value.content_account}</p>
                                    <p>${value.comment}</p>
                                    <p>${value.getday}</p>
                                </div>
                            `;
                        if (contentid == value.content_id) {
                            $(commnenttarget).append(commentTag);
                        }
                    });
                },
                // 通信失敗時
    ...
    $('.send').on('click', function () {
        ...
        const contentid = $(this)
            ...
            .text();
        const commentdata = {
            commentdata: $(this).siblings('.commentinput').val(),
            contentid: contentid,
        };
        ...
        $.ajax({
            ...
            function (data) {
                ...
                console.log(respons_commentdata);
                    const commentTag = `
                                <div class="commentcontent">
                                    <p>${respons_commentdata.content_account}</p>
                                    <p>${respons_commentdata.comment}</p>
                                    <p>${respons_commentdata.getday}</p>
                                </div>
                            `;
                    if (contentid == respons_commentdata.content_id) {
                        $(commnenttarget).append(commentTag);
                    }
            },
            // 通信失敗時
